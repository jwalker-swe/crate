...
/**
 * Calculates the average album rating using only the most recent rating from each user
 * @param albumId - The database ID of the album
 * @returns The calculated average rating, or null if no ratings exist
 */
export default async function calculateAlbumRating(albumId: string): Promise<number | null> {
