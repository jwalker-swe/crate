...
export default function EditListModal({ isOpen, onClose, list }: EditListModalProps) {
	...
	// Initialize selected albums from list data
	useEffect(() => {
		if (isOpen) {
			setName(list.name)
			...
			const albums = list.list_albums
				.sort((a, b) => a.position - b.position)
				.filter(la => la.albums)
				.map(la => {
					const albumInfo = Array.isArray(la.albums) ? la.albums[0] : la.albums!
					return {
						spotify_id: albumInfo.spotify_id,
						name: albumInfo.title,
						artist: albumInfo.artist || 'Unknown Artist',
						imageUrl: albumInfo.cover_image_url,
						dbId: albumInfo.id
					}
				})
	...
	async function handleSubmit() {
		...
		try {
			...
			// Delete existing list_albums and re-add them
			...
			if (deleteError) {
				console.error('Error removing old albums:', deleteError)
				setError('Failed to update list albums. Please try again.')
				setIsSubmitting(false)
				return
			}
			...
			if (selectedAlbums.length > 0) {
				...
				for (let i = 0; i < selectedAlbums.length; i++) {
					...
					if (dbId) {
						...
						console.error(`Failed to get/create album in DB: ${album.name}`)
					}
				}
				...
				if (albumInserts.length > 0) {
					...
					if (albumsError) {
						...
						setError('Failed to save album changes. Please try again.')
						setIsSubmitting(false)
						return
					}
				}
			...
			console.error('Unexpected error updating list:', err)
		...
		>
			...
			>
				...
				<div className="flex items-center justify-between p-4 border-b border-primaryBorder flex-shrink-0">
					<h2 className="text-xl font-semibold text-primaryText">Edit List</h2>
					<button
						onClick={handleClose}
				...
				<div ref={contentRef} className="flex-1 overflow-y-auto p-4 space-y-6">
					...
					{selectedAlbums.length > 0 && (
						<div>
							<label className="block text-sm font-medium text-primaryText mb-2">
								Albums in List ({selectedAlbums.length})
							</label>
					...
					<div className="pt-4 border-t border-primaryBorder">
						...
						{!showDeleteConfirm ? (
							<button
								...
								className="px-4 py-2 text-red-400 border border-red-400/30 rounded-lg hover:bg-red-400/10 transition-colors text-sm"
							>
								Delete List
							</button>
						) : (
							<div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
								...
								<div className="flex gap-3">
									...
									<button
										...
										className="px-4 py-2 text-secondaryText hover:text-primaryText transition-colors text-sm"
									>
										Cancel
									</button>
								</div>
